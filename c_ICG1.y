...
%token DOT SINGLE SC  COMMA LETTER  OPBRACE CLBRACE CONTINUE BREAK IF ELSE FOR DO WHILE POW OPEN CLOSE COMMENT SQ_OPEN SQ_CLOSE
...
%token <text> ID NUM ALPHA PLUS MINUS MULT DIV AND OR LESS GREAT LESEQ GRTEQ NOTEQ EQEQ ASSIGN SPLUS SMINUS SMULT SDIV INC DEC SWITCH
...
C   : CASE NUM COLON stat      {char* lab1 = new_label();
	...
	|
	CASE ALPHA COLON stat      {char* lab1 = new_label();
								//printf("in switch\n"); 
								char* lab2 = new_label(); 
								//printf("in switch\n"); 
								$$.code = code_concatenate(6,code_gen(4,"if ",$<text>-2," == ",$2," goto ", lab1), " goto ",lab2, code_gen(2,lab1,": "),$4.code, code_gen(2,lab2,": "));
...
iter_stat:DO comp_stat WHILE OPEN cond CLOSE SC		{char* begin = new_label(); 
															...
															strcpy(break_lab,$2.next);
															//printf("here2 %s",break_lab);
															//$2.next = new_label();
															

															$$.code = code_concatenate(5, code_gen(2,begin,": "), $2.code, code_gen(4,"if ", $5.code, "goto ", $5.true), 
															code_gen(2,"goto ", $5.false), code_gen(2,$2.next,": \n"));

															if(num_iter)
															{
																//printf("HERE\n");
																$$.optimized_code = " ";
																$$.optimized_code = code_concatenate(1,$$.optimized_code);
																...
																{
																	$$.optimized_code = code_concatenate(2,$$.optimized_code,$2.optimized_code);
																}
...
relexp:E relOp E  {$$.code = code_gen(3,$1.code,$3.code,code_gen(3,$1.addr,$2,$3.addr)); 
					...
					if(strcmp($2,"<")==0)
					{
						...
						// printf("TOP: %d\n",iter_top); a<b b-a num_iter stack for b, iter_init stack for a
						number_of_iterations = atoi($3.addr)-temp;
					...
					if(strcmp($2,"<=")==0)
					{
						// num_iter[iter_top++]=atoi($3.addr);
						// printf("TOP: %d\n",iter_top);
						number_of_iterations = atoi($3.addr)-temp+1;
						// printf("Number of iterations: %d\n",number_of_iterations);
					}
					if(strcmp($2,">")==0)
					{
						// num_iter[iter_top++]=atoi($3.addr);
						// printf("TOP: %d\n",iter_top);
						number_of_iterations = temp - atoi($3.addr);
						// printf("Number of iterations: %d\n",number_of_iterations);
					}
					if(strcmp($2,">=")==0)
					{
						// num_iter[iter_top++]=atoi($3.addr);
						// printf("TOP: %d\n",iter_top);
						number_of_iterations = temp - atoi($3.addr)+1;
						// printf("Number of iterations: %d\n",number_of_iterations);
					}
...
F:ID  {$$.addr = gen_addr($1); $$.code = code_gen(1," ");if(!look_up_sym_tab($1)){printf("Undeclared variable %s\n", $1); YYERROR;} $$.optimized_code = $$.code;$$.is_dig=0;}
 ...
 |ALPHA    {$$.addr = gen_addr($1); $$.code = code_gen(1," ");$$.optimized_code = $$.code; $$.val = atof($1);$$.is_dig=0;} 
 |OPEN E CLOSE   {$$= $2;}
 |unary_expr      {$$=$1;}
 |s_operation     {$$=$1;}
 ;
